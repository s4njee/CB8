/**
 * metadataScraper.ts — search external metadata sources (ComicVine, AniList,
 * MangaDex) for a comic/book candidate match.
 *
 * Each adapter is best-effort: network errors or missing API keys produce a
 * warning rather than a hard error. Results are a flat list of candidates the
 * client presents to the admin for selection.
 */
