...
export const handle: RouteHandler = async (ctx) => {
  ...
  if (method === 'PUT' && progressMatch) {
    ...
    if (typeof opts.page === 'number' && opts.completed === undefined) {
      const comic = db.comics.getComic(id);
      if (comic && comic.pageCount > 0 && opts.page >= comic.pageCount - 1) {
        opts.completed = true;
      }
    }
    db.progress.upsertUserProgress(currentUser.id, id, opts);
    if (typeof parsed.page === 'number') db.comics.updateReadingProgress(id, parsed.page);
    else if (typeof parsed.location === 'string') db.comics.updateReadingLocation(id, parsed.location);
  ...
  if (method === 'DELETE' && progressMatch) {
    if (!currentUser) { sendError(res, 401, 'Unauthorized'); return true; }
    const id = parseInt(progressMatch[1], 10);
    db.progress.clearUserProgress(currentUser.id, id);
    sendJson(res, 200, { ok: true });
    return true;
  }
  ...
  if (favMatch && (method === 'POST' || method === 'DELETE')) {
    ...
    if (method === 'POST') db.favorites.addFavorite(currentUser.id, id);
    else db.favorites.removeFavorite(currentUser.id, id);
    sendJson(res, 200, { ok: true });
    return true;
  }
  ...
  if (method === 'GET' && bookmarksMatch) {
    ...
    sendJson(res, 200, db.bookmarks.listBookmarks(currentUser.id, id));
    return true;
  }
  if (method === 'POST' && bookmarksMatch) {
    ...
    sendJson(res, 201, db.bookmarks.createBookmark(currentUser.id, id, parsed.page, parsed.note ?? null));
    return true;
  }
  ...
  if (method === 'PUT' && bookmarkItemMatch) {
    ...
    try { parsed = JSON.parse(body); } catch { sendError(res, 400, 'Invalid JSON'); return true; }
    db.bookmarks.updateBookmark(currentUser.id, bookmarkId, parsed.note ?? null);
    sendJson(res, 200, { ok: true });
    return true;
  }
  if (method === 'DELETE' && bookmarkItemMatch) {
    if (!currentUser) { sendError(res, 401, 'Unauthorized'); return true; }
    const bookmarkId = parseInt(bookmarkItemMatch[2], 10);
    db.bookmarks.deleteBookmark(currentUser.id, bookmarkId);
    sendJson(res, 200, { ok: true });
    return true;
  }
  ...
  if (method === 'POST' && pathname === '/api/history') {
    ...
    db.history.logHistory(currentUser.id, parsed.comicId, parsed.action, parsed.page ?? null);
    sendJson(res, 200, { ok: true });
    return true;
  }
  if (method === 'GET' && pathname === '/api/history') {
    ...
    sendJson(res, 200, db.history.getHistory(currentUser.id, offset, limit));
    return true;
  }
  ...
  if (method === 'GET' && pathname === '/api/recently-read') {
    ...
    const records = currentUser
      ? db.progress.getRecentlyReadByUser(currentUser.id, limit, mediaType)
      : db.comics.getRecentlyRead(limit, mediaType);
  ...
  if (method === 'GET' && pathname === '/api/continue-reading') {
    ...
    const mediaType = query.mediaType as 'comic' | 'book' | undefined;
    const records = currentUser
      ? db.progress.getContinueReadingByUser(currentUser.id, limit, mediaType)
      : db.comics.getContinueReading(limit, mediaType);
    sendJson(res, 200, records.map(toWebRecord));
    return true;
  }
