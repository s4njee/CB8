...
/**
 * Opens the add-from-server-path flow directly (used by the "+" button in
 * the nav). The button is only visible to superadmins (see CSS), so we go
 * straight to the add-path modal without an auth fallback.
 */
export function openAddComic() {
  openModal(renderAddPath);
}
